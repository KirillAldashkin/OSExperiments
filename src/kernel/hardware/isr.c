...
{
	string intr = "Received interrupt 0x        ";
	string code = "Error code 0x        ";
	string insc = "Instruction 0x        ";
	ToString(registers.InterruptNumber, 16, intr+21);
	ToString(registers.ErrorCode, 16, code+13);
	ToString(registers.Eip, 16, insc+14);
	WriteLine(intr);
	WriteLine(code);
	WriteLine(insc);
}
